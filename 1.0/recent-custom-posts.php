...
class rcp_recent_posts extends WP_Widget {
	...
	function rcp_recent_posts() {
		parent::WP_Widget( 'widget_recent_custom_posts', $name = 'Recent Custom Posts' );
	}
	...
	**/
	function widget( $args, $instance ) {
		...
		echo $before_widget;
			...
			if( $mlq->have_posts() ) : 
			?>
			<ul id="recent_custom_posts_list">
			<?php while($mlq->have_posts()) : $mlq->the_post(); ?>
			<li><a href="<?php the_permalink(); ?>" rel="bookmark" class="clearfix"><?php the_title(); ?></a></li>
			<?php wp_reset_query(); 
			endwhile; ?>
			</ul>
	...
	function update( $new_instance, $old_instance ) {
		...
		$types_array = $new_instance['types'];
		$types       = implode(',', (array)$types_array);;
	...
	**/
	function form( $instance ) {	
			...
			$cpt_types = get_post_types( $arg, 'names' );
